...
function Scene(mode,cam,cnvs,stretches,center){
    let cnvs_=null,mode_=0,cam_=identityMatrix(4),center_=vec3(),stretches_=vec3(1,1,1);
    if(cnvs){cnvs_=cnvs;}
    if(mode){mode_=mode;}
    if(cam){cam_=cam;}
    if(center){center_=center}
    if(stretches){stretches_=stretches;}
  return {'mode':mode_,'cam':cam_,'cnv':cnvs_,'stretches':stretches_,'center':center_}
}
function Rotor(arr){
  let obj=vec3();
  if (arr){
...
function pol2cart(r,theta,plane,xdir){
  ...
  if (plane){
    let a=plane.x,b=plane.y,c=plane.z;
    let ux;
    if (xdir){ux=xdir;}
    ...
    let uy=vecCross(plane,ux);
    ux=vecScale(uy,Math.cos(theta));
    uy=vecScale(uy,Math.sin(theta));
    posV=vecScale(vecSum(ux,uy),r);
  }
  else{
    posV=new arr2vec([r*Math.cos(theta),r*Math.sin(theta)]);
  }
  return posV;
}
//---------------
...
function homogenize(vecObj){
  return [vecObj.x,vecObj.y,vecObj.z,1];
}
